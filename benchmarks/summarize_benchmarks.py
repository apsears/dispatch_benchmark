...
def load_results(results_dir, iso=None):
    ...
    print(
        f"No combined results file found. Looking for individual files in {results_dir}"
    )
    all_results = []
...
def calculate_metrics(results_df, iso=None):
    ...
    grouped = (
        results_df.groupby(groupby_cols)
        .agg({"revenue": "mean", "runtime_seconds": "mean"})
        .reset_index()
    )
    ...
    pivot_runtime = grouped.pivot(
        index=pivot_index, columns="model", values="runtime_seconds"
    )
    ...
    if "oracle_lp" in pivot_revenue.columns:
        for model in pivot_revenue.columns:
            if model != "oracle_lp":
                ...
                relative_df[f"{model}_vs_oracle"] = (
                    pivot_revenue[model] / pivot_revenue["oracle_lp"]
                ) * 100

        # Calculate difference between ridge and naive if both exist
        if (
            "online_mpc_ridge" in pivot_revenue.columns
            and "online_mpc_naive" in pivot_revenue.columns
        ):
            relative_df["ridge_vs_naive"] = (
                (pivot_revenue["online_mpc_ridge"] - pivot_revenue["online_mpc_naive"])
                / pivot_revenue["online_mpc_naive"]
            ) * 100
    ...
    if "iso" in grouped.columns:
        ...
        for iso_name, iso_group in grouped.groupby("iso"):
            ...
            iso_relative_df = (
                relative_df.loc[iso_name] if not relative_df.empty else pd.DataFrame()
            )
            ...
            summary_stats[iso_name] = {
                ...
                "relative_means": {
                    col: iso_relative_df[col].mean() for col in iso_relative_df.columns
                },
                "relative_stdev": {
                    col: iso_relative_df[col].std() for col in iso_relative_df.columns
                },
            }
        ...
        summary_stats["OVERALL"] = {
            ...
            "relative_means": {
                col: relative_df[col].mean() for col in relative_df.columns
            },
            "relative_stdev": {
                col: relative_df[col].std() for col in relative_df.columns
            },
        }
    else:
        summary_stats = {
...
def print_summary(metrics, iso=None):
    ...
    if isinstance(metrics["summary"], dict) and any(
        key in metrics["summary"] for key in ["ERCOT", "NYISO", "OVERALL"]
    ):
        ...
        for current_iso in isos_to_print:
            ...
            revenue_df = pd.DataFrame(
                {
                    "Mean Revenue ($)": metrics["summary"][current_iso][
                        "revenue_means"
                    ],
            ...
            if metrics["summary"][current_iso]["relative_means"]:
                ...
                for model, pct in metrics["summary"][current_iso][
                    "relative_means"
                ].items():
                    if "vs_oracle" in model:
                        ...
                        rel_data.append(
                            [
                                model_name,
                                pct,
                                metrics["summary"][current_iso]["relative_stdev"][
                                    model
                                ],
                            ]
                        )
            ...
            if "ridge_vs_naive" in metrics["summary"][current_iso]["relative_means"]:
                ridge_vs_naive = metrics["summary"][current_iso]["relative_means"][
                    "ridge_vs_naive"
                ]
                ridge_vs_naive_std = metrics["summary"][current_iso]["relative_stdev"][
                    "ridge_vs_naive"
                ]
                ...
                if current_iso == "OVERALL":
                    ...
                    for iso_name in [
                        k for k in metrics["summary"].keys() if k != "OVERALL"
                    ]:
                        if iso_name in metrics["relative"].index.get_level_values(0):
                            ...
                            print(
                                f"{iso_name}: Ridge outperforms Naive in {ridge_wins} out of {total_nodes} nodes ({ridge_wins/total_nodes*100:.1f}%)"
                            )
                else:
                    ...
                    if current_iso in metrics["relative"].index.get_level_values(0):
                        ...
                        total_nodes = len(iso_relative_df)
                        print(
                            f"Ridge outperforms Naive in {ridge_wins} out of {total_nodes} nodes ({ridge_wins/total_nodes*100:.1f}%)"
                        )
        ...
        if metrics["summary"]["relative_means"]:
            ...
            for model, pct in metrics["summary"]["relative_means"].items():
                if "vs_oracle" in model:
                    model_name = model.replace("_vs_oracle", "")
                    rel_data.append(
                        [model_name, pct, metrics["summary"]["relative_stdev"][model]]
                    )
...
def visualize_results(metrics, output_dir, iso=None):
    ...
    if isinstance(metrics["summary"], dict) and any(
        ...
        for current_iso in isos_to_visualize:
            ...
            summary = metrics["summary"][current_iso]["revenue_means"].sort_values(
                ascending=False
            )
            summary.plot(
                kind="bar", yerr=metrics["summary"][current_iso]["revenue_stdev"]
            )
            ...
            if metrics["summary"][current_iso]["relative_means"]:
                ...
                rel_std = pd.Series(
                    {
                        k.replace("_vs_oracle", ""): v
                        for k, v in metrics["summary"][current_iso][
                            "relative_stdev"
                        ].items()
                        if "vs_oracle" in k
                    }
                ...
                plt.xlabel("Model")
                plt.axhline(
                    y=100, color="r", linestyle="--", alpha=0.3
            ...
            plt.figure(figsize=(10, 6))
            metrics["summary"][current_iso]["runtime_means"].sort_values().plot(
                kind="bar"
            )
...
def main():
    ...
    if args.run_benchmarks:
        ...
        if args.iso in ["NYISO", "BOTH"]:
            nyiso_success = run_benchmark_for_iso(
                "NYISO",
                "data/nyiso/2024_RealTime_LBMP.csv",
                args.output_dir,
                args.max_nodes,
                args.n_jobs,
            )
    ...
    if args.iso == "BOTH":
        ...
        if args.iso == "ERCOT":
            ...
            if ercot_results is not None:
                ...
                create_combined_csv(
                    ercot_metrics, os.path.join(args.output_dir, "ercot_summary")
                )
            ...
            nyiso_results_dir = args.results_dir
            # Check if results are in the benchmark_summary/nyiso directory
            if not (Path(nyiso_results_dir) / "combined_results.json").exists():
                nyiso_results_dir = "benchmark_summary/nyiso"
                if not (Path(nyiso_results_dir) / "combined_results.json").exists():
                    print("No NYISO results found")
                    return

            nyiso_results = load_results(nyiso_results_dir, "NYISO")
            if nyiso_results is not None:
                nyiso_metrics = calculate_metrics(nyiso_results)
                print("\n=== NYISO BENCHMARK RESULTS ===")
                print_summary(nyiso_metrics, "NYISO")
                create_combined_csv(
                    nyiso_metrics, os.path.join(args.output_dir, "nyiso_summary")
                )
    ...
    if (
        args.iso == "BOTH"
        and "metrics" in locals()
        and isinstance(metrics["summary"], dict)
        and "ERCOT" in metrics["summary"]
        and "NYISO" in metrics["summary"]
    ):
        print("\n=== ERCOT vs NYISO COMPARISON ===")

        # Compare oracle performances
        ercot_oracle = metrics["summary"]["ERCOT"]["revenue_means"].get(
            "oracle_lp", np.nan
        )
        nyiso_oracle = metrics["summary"]["NYISO"]["revenue_means"].get(
            "oracle_lp", np.nan
        )

        if not np.isnan(ercot_oracle) and not np.isnan(nyiso_oracle):
            print(
                f"Average Oracle Revenue: ERCOT ${ercot_oracle:.2f} vs NYISO ${nyiso_oracle:.2f}"
            )
            print(f"NYISO/ERCOT Revenue Ratio: {nyiso_oracle/ercot_oracle:.2f}x")

        # Compare ridge forecaster performance relative to oracle
        ercot_ridge = metrics["summary"]["ERCOT"]["relative_means"].get(
            "online_mpc_ridge_vs_oracle", np.nan
        )
        nyiso_ridge = metrics["summary"]["NYISO"]["relative_means"].get(
            "online_mpc_ridge_vs_oracle", np.nan
        )

        if not np.isnan(ercot_ridge) and not np.isnan(nyiso_ridge):
            print(
                f"Ridge vs Oracle: ERCOT {ercot_ridge:.2f}% vs NYISO {nyiso_ridge:.2f}%"
            )

        # Compare ridge vs naive improvement
        ercot_vs_naive = metrics["summary"]["ERCOT"]["relative_means"].get(
            "ridge_vs_naive", np.nan
        )
        nyiso_vs_naive = metrics["summary"]["NYISO"]["relative_means"].get(
            "ridge_vs_naive", np.nan
        )

        if not np.isnan(ercot_vs_naive) and not np.isnan(nyiso_vs_naive):
            print(
                f"Ridge improvement over Naive: ERCOT {ercot_vs_naive:.2f}% vs NYISO {nyiso_vs_naive:.2f}%"
            )
