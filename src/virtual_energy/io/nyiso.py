...
def parse_args():
    ...
    parser.add_argument(
        "--year", type=int, required=True, help="Year to process (YYYY)"
    )
    parser.add_argument(
        "--market",
        choices=["da", "rt"],
...
def get_months_for_year(year):
    ...
    for month in range(1, 13):
        ...
        if (
            month_date.year == current_date.year
            and month_date.month == current_date.month
        ):
            continue
...
def combine_processed_data(
    processed_dfs, year, market_type, output_dir, wide_format=False
):
    ...
    try:
        ...
        if wide_format:
            ...
            output_path = os.path.join(
                output_dir, f"{year}_{market_name}_LBMP_wide.csv"
            )
        ...
        if wide_format:
            print(
                f"Data shape: {final_df.shape[0]} rows (days) x {final_df.shape[1]} columns"
            )
            print(
                f"Number of unique zones: {final_df.shape[1] - 1}"
            )  # Subtract 1 for the date column
        else:
...
def main(args=None):
    ...
    )  # Make wide_format optional for backward compatibility

    # Ensure it's a valid year
    current_year = datetime.date.today().year
    if year < 2000 or year > current_year:
        print(
            f"Error: Year {year} is out of range. Must be between 2000 and {current_year}."
        )
        return
    ...
    download_tasks = []
    for url, filename, month_display in generate_monthly_urls(
        months, file_format, market_type
    ):
    ...
    final_df, market_name = combine_processed_data(
        processed_dfs, year, market_type, output_dir, wide_format=wide_format
    )
    ...
    if final_df is not None:
        ...
        return (
            output_dir
            / f"{year}_{market_name}_LBMP{'_wide' if wide_format else ''}.csv"
        )
...
def load_prices(start, end, nodes=None, freq="1D") -> pd.DataFrame:
        ...
        # If tidy format not available, try wide format
            ...
            if timestamp_col:
                ...
                tidy_df = wide_df.melt(
                    id_vars=[timestamp_col], var_name="node", value_name="price"
                )
                ...
                if csv_files:
                    ...
                    for csv_file in csv_files:
                        ...
                        try:
                            ...
                            if (
                                "Time Stamp" in raw_df.columns
                                and "Name" in raw_df.columns
                                and "LBMP ($/MWHr)" in raw_df.columns
                            ):
                                df = raw_df[
                                ...
                                print(
                                    f"Expected columns not found in {csv_file}, skipping"
                                )
    ...
    result_df = result_df[
        (result_df["timestamp"] >= start_date) & (result_df["timestamp"] <= end_date)
    ]
...
def infer_frequency(df):
    ...
    if "node" in df.columns:
        ...
        timestamps = (
            df[df["node"] == sample_node]["timestamp"]
            .sort_values()
            .reset_index(drop=True)
        )
    else:
