...
def test_soc_invariant(random_prices, battery_config):
    ...
    - SoC never exceeds battery capacity (200 MWh)
    """
    ...
    assert dispatch["SoC_MWh"].min() >= 0, f"SoC goes below 0: {dispatch['SoC_MWh'].min()}"
    assert dispatch["SoC_MWh"].max() <= battery_config.e_max_mwh, \
    ...
    assert (soc_diff != 0).any(), "SoC doesn't change - battery never charges or discharges"
    
    # Verify charge and discharge power don't exceed maximum power
    assert dispatch["p_discharge_MW"].max() <= battery_config.p_max_mw, \
        f"Discharge power exceeds max: {dispatch['p_discharge_MW'].max()} > {battery_config.p_max_mw}"
    assert dispatch["p_charge_MW"].max() <= battery_config.p_max_mw, \
