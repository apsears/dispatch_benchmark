...
def test_soc_invariant(random_prices, battery_config):
    ...
    _, dispatch = create_and_solve_model(
        random_prices, battery_config
    )  # No longer returns solver_status

    # Get the SOC values
    soc = dispatch[
        "SoC_MWh"
    ]  # Update to match the column name in the returned dataframe
    ...
    assert (
        soc.max() <= battery_config.e_max_mwh
    ), f"SOC exceeded battery capacity: maximum value {soc.max()}"
    ...
    ), "Discharging power exceeded limits"
    assert (
        dispatch["p_charge_MW"].max() <= battery_config.p_max_mw
    ), "Charging power exceeded limits"
    ...
    if "MWhDeployed" in dispatch.columns:
        ...
        assert (
            mwh_diff.max() < 1e-6
        ), f"MWh deployed calculation incorrect, max diff: {mwh_diff.max()}"
